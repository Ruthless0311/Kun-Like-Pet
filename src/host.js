...
const CONFIG = {
  ...
  spritePath: 'D:/harness-UI/kun-like-pet/assets/spritesheet.webp',
  // 任务完成提示音路径（mp3）
  ...
  // 宿主进程系统级播放命令（macOS 用 afplay；Windows 可用 powershell -c (New-Object Media.SoundPlayer '...').PlaySync()；Linux 可用 ffplay -nodisp -autoexit）
    playCommand: (path) => "powershell -c (New-Object System.Media.SoundPlayer '" + path.replace(/'/g, "''") + "').PlaySync()",
  // 状态轮询间隔（毫秒）
  pollMs: 500,
  // 庆祝动画持续时长（毫秒）
  celebrateMs: 4800,
